...
* If you made a **change to a submodule** `base_lib` (located in `<main-repo-path>/external/base_lib`) then you should see it with `git status`. You should commit the submodule change, then the change into the main repository.
...
* When you are **pushing**, if you changed a submodule, you should make sure to also push it. Use `git spush` in the main repository, it will take care of that.
