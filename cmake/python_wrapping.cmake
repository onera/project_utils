...
function(compile_install_pybind_module project_name)
    # 1. Définition des arguments acceptés
    set(options "")
    set(oneValueArgs "")
    set(multiValueArgs EXTRA_DIRS)
    cmake_parse_arguments(PYBIND "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    # 2. Préparation des patterns de recherche
    # On commence par le chemin par défaut (ton projet de base)
    set(search_patterns "${CMAKE_CURRENT_SOURCE_DIR}/${project_name}/*.pybind.cpp")

    # 3. Ajout des chemins supplémentaires s'ils existent
    foreach(dir IN LISTS PYBIND_EXTRA_DIRS)
        list(APPEND search_patterns "${CMAKE_CURRENT_SOURCE_DIR}/${dir}/*.pybind.cpp")
    endforeach()

    # 4. Collecte des fichiers avec les patterns accumulés
    file(GLOB_RECURSE pybind_files CONFIGURE_DEPENDS ${search_patterns})

    # Le reste ne change pas (ou presque)
    set(mod_name c${project_name})

    if(NOT pybind_files)
        message(WARNING "Aucun fichier .pybind.cpp trouvé pour ${mod_name}")
        return()
    endif()
